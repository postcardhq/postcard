...
import { Clock, Fingerprint, Globe, CaretDown, MapTrifold } from '@phosphor-icons/react'
...
function scoreVerdict(s: number) {
  ...
  if (s >= 0.5) return 'Unreliable Postmark'
  return 'Fabricated'
}
...
function CancelStamp({ className }: { className?: string }) {
  return (
    <svg viewBox="0 0 120 120" className={className} fill="none" aria-hidden>
      <circle cx="60" cy="60" r="55" stroke="var(--postal-red)" strokeWidth="3" />
      <circle cx="60" cy="60" r="46" stroke="var(--postal-red)" strokeWidth="1.2" />
      <circle cx="60" cy="60" r="37" stroke="var(--postal-red)" strokeWidth="1" />
      {[40, 50, 60, 70, 80].map((y) => (
        <line key={y} x1="5" y1={y} x2="115" y2={y} stroke="var(--postal-red)" strokeWidth="2.5" />
      ))}
      <path id="cancel-top" d="M 18,55 A 42,42 0 0,1 102,55" fill="none" />
      <path id="cancel-bot" d="M 18,65 A 42,42 0 0,0 102,65" fill="none" />
      <text fontSize="7.5" fill="var(--postal-red)" fontFamily="var(--font-serif)">
        <textPath href="#cancel-top" startOffset="8%">POSTCARD FORENSICS</textPath>
      </text>
      <text fontSize="7.5" fill="var(--postal-red)" fontFamily="var(--font-serif)">
        <textPath href="#cancel-bot" startOffset="8%">VERIFIED ✦ AUTHENTICATED</textPath>
      </text>
    </svg>
  )
}
...
function ScoreGauge({
  ...
  return (
    <div className="flex flex-col gap-1.5">
      ...
      <div className="h-1.5 w-full rounded-none" style={{ background: 'var(--postal-paper-3)' }}>
        <motion.div
      ...
      <div className="flex flex-col items-center">
        ...
        {!isLast && (
          <div
            ...
            style={{ borderColor: 'var(--postal-ink-faint)', minHeight: '28px' }}
          />
        )}
      </div>
      {/* Content */}
...
export function ForensicReport({ report }: { report: PostcardReport }) {
  ...
  return (
    <div
      ...
      style={{ background: 'var(--postal-paper)', fontFamily: 'var(--font-serif)' }}
    >
      ...
      <div className="mx-auto max-w-2xl px-6 py-12 flex flex-col gap-8">
        ...
        <section className="flex flex-col items-center gap-5 text-center">
          ...
          {/* Rubber stamp score — scale 1.5 → 1, rotate -5deg → 0 */}
          <motion.div
          ...
          {/* Platform / timestamp / username stamps */}
          <motion.div
          ...
          >
            <MetaStamp
              icon={<Globe size={11} style={{ color: 'var(--postal-ink-muted)' }} />}
              text={ocr.postmark.platform}
            />
            {ocr.postmark.timestampText && (
              <MetaStamp
                icon={<Clock size={11} style={{ color: 'var(--postal-ink-muted)' }} />}
                text={ocr.postmark.timestampText}
              />
            )}
            {ocr.postmark.username && (
              <MetaStamp
                icon={<Fingerprint size={11} style={{ color: 'var(--postal-ink-muted)' }} />}
        ...
        {/* Divider */}
        <div className="h-px" style={{ background: 'var(--postal-ink-faint)' }} />
        ...
        >
          ...
          {/* Node 1: Origin */}
          <TimelineNode
            icon={<MapTrifold size={13} style={{ color: 'var(--postal-ink-muted)' }} />}
            label="Origin"
          >
            {triangulation.targetUrl ? (
              ...
              <p className="text-xs italic" style={{ color: 'var(--postal-ink-faint)' }}>
          ...
          {/* Node 3: Forensic Breakdown */}
          <TimelineNode
            icon={<Fingerprint size={13} style={{ color: 'var(--postal-ink-muted)' }} />}
            label="Forensic Breakdown"
            isLast
          >
            <div className="grid grid-cols-3 gap-4">
              <ScoreGauge label="Origin"   value={audit.originScore}   delay={0.9} />
              <ScoreGauge label="Temporal" value={audit.temporalScore} delay={1.0} />
              <ScoreGauge label="Visual"   value={audit.visualScore}   delay={1.1} />
            </div>
            <p
        ...
        >
          <Accordion.Root type="single" collapsible>
            <Accordion.Item value="semantic-diff">
              <Accordion.Trigger
                className="fr-accordion-trigger w-full flex items-center justify-between cursor-pointer px-3 py-2"
                style={{
                  background: 'var(--postal-paper-2)',
                  border: '1px solid var(--postal-ink-faint)',
                  fontFamily: 'var(--font-serif)',
              ...
              <Accordion.Content
                className="fr-accordion-content"
                style={{
              ...
              >
                <div className="flex flex-col gap-4 p-4">
                  ...
                  <div>
                    ...
                    >
                      <p>
                        <span className="font-semibold" style={{ color: 'var(--postal-ink)' }}>
                          Main text:
                        </span>{' '}
                      ...
                      {ocr.postmark.uiAnchors && ocr.postmark.uiAnchors.length > 0 && (
                        <p className="mt-1">
                          <span className="font-semibold" style={{ color: 'var(--postal-ink)' }}>
                            UI Anchors:
                          </span>{' '}
                          {ocr.postmark.uiAnchors
                            .map((a) => `${a.element} (${a.position}, ${Math.round(a.confidence * 100)}%)`)
                            .join(' · ')}
                        </p>
                      )}
                    </div>
